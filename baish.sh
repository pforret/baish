...
readonly script_description="bash tools for AI agents"
...
function Script:main() {
  ...
  case "${action,,}" in
  tokens)
    #TIP: use «$script_prefix tokens» to estimate LLM token count
    #TIP:> $script_prefix tokens "your text here"
...
function do_tokens() {
  ...
  # Estimate tokens using combined method
  local tokens_by_chars tokens_by_words tokens_combined
  tokens_by_chars=$(Tool:calc "$char_count / $chars_per_token")
  tokens_by_words=$(Tool:calc "$word_count * $words_multiplier")
  tokens_combined=$(Tool:calc "($tokens_by_chars + $tokens_by_words) / 2")

  # Round to integer
  local estimated_tokens
  estimated_tokens=$(Tool:round "$tokens_combined" 0)
...
function detect_syntax() {
  ...
  case "$ext" in
    ...
    txt|md|markdown|rst|text|doc|docx|rtf)
      echo "prose"
      ;;
    # Programming languages
    py|python|js|javascript|ts|typescript|go|rs|rust|php|pl|perl|rb|ruby|java|kt|kotlin|scala|c|cpp|cc|cxx|h|hpp|cs|csharp|swift|m|mm|r|lua|zig|nim|d|f90|f95|fortran|ex|exs|elixir|erl|erlang|hs|haskell|ml|ocaml|fs|fsharp|clj|clojure|lisp|scm|scheme|rkt|racket|v|verilog|vhdl|sql)
      echo "code"
      ;;
    # Shell scripts
    sh|bash|zsh|fish|ksh|csh|tcsh|ps1|psm1|bat|cmd)
      ...
      ;;
    # Markup languages
    html|htm|xhtml|css|scss|sass|less|xml|svg|vue|jsx|tsx|svelte)
      echo "markup"
      ;;
    # Data formats
    json|csv|tsv|yaml|yml|toml|ini|conf|cfg|env|properties|plist)
      echo "data"
      ;;
    # Default to prose
    *)
